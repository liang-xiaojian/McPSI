...
std::vector<ATy> AddAP([[maybe_unused]] std::shared_ptr<Context>& ctx,
  ...
  if (ctx->GetRank() == 0) {
    vec64::Add(absl::MakeConstSpan(val), absl::MakeConstSpan(rhs),
               absl::MakeSpan(val));
  }
  std::vector<PTy> rhs_mac(size);
  vec64::ScalarMul(ctx->GetState<Protocol>()->GetKey(),
                   absl::MakeConstSpan(rhs), absl::MakeSpan(rhs_mac));
  vec64::Add(absl::MakeConstSpan(mac), absl::MakeConstSpan(rhs_mac),
             absl::MakeSpan(mac));
...
std::vector<ATy> MulAP([[maybe_unused]] std::shared_ptr<Context>& ctx,
  ...
  vec64::Mul(absl::MakeConstSpan(rhs), absl::MakeConstSpan(val),
             absl::MakeSpan(val));
  vec64::Mul(absl::MakeConstSpan(rhs), absl::MakeConstSpan(mac),
             absl::MakeSpan(mac));
  return Pack(absl::MakeSpan(val), absl::MakeSpan(mac));
}
...
std::vector<ATy> DivAP([[maybe_unused]] std::shared_ptr<Context>& ctx,
  ...
  vec64::Inv(absl::MakeConstSpan(rhs), absl::MakeSpan(inv));
...
std::vector<PTy> A2P(std::shared_ptr<Context>& ctx, absl::Span<const ATy> in) {
  ...
  if (ctx->GetRank() == 0) {
    ...
    vec64::Add(
        absl::MakeConstSpan(reinterpret_cast<const PTy*>(buf.data()), size),
  ...
  auto coef = vec64::Rand(sync_seed, size);
  // linear combination
  auto real_val_affine =
      vec64::InPro(absl::MakeSpan(coef), absl::MakeSpan(real_val));
  auto mac_affine = vec64::InPro(absl::MakeSpan(coef), absl::MakeSpan(mac));
...
std::vector<ATy> P2A(std::shared_ptr<Context>& ctx, absl::Span<const PTy> in) {
  ...
  if (ctx->GetRank() == 0) {
    vec64::Add(absl::MakeConstSpan(zero_val), absl::MakeConstSpan(in),
               absl::MakeSpan(zero_val));
  }
  auto in_mac = vec64::ScalarMul(ctx->GetState<Protocol>()->GetKey(), in);
  vec64::Add(absl::MakeConstSpan(zero_mac), absl::MakeConstSpan(in_mac),
...
std::vector<ATy> ShuffleAGet(std::shared_ptr<Context>& ctx,
  ...
  vec64::Add(absl::MakeConstSpan(val_a), absl::MakeConstSpan(val_in),
             absl::MakeSpan(val_a));
  vec64::Add(absl::MakeConstSpan(mac_a), absl::MakeConstSpan(mac_in),
             absl::MakeSpan(mac_a));
...
std::vector<ATy> ShuffleASet(std::shared_ptr<Context>& ctx,
  ...
  auto [val_in, mac_in] = Unpack(absl::MakeConstSpan(in));
  vec64::Add(absl::MakeConstSpan(val_in), absl::MakeConstSpan(val_tmp),
             absl::MakeSpan(val_tmp));
  vec64::Add(absl::MakeConstSpan(mac_in), absl::MakeConstSpan(mac_tmp),
...
std::vector<ATy> SetA(std::shared_ptr<Context>& ctx, absl::Span<const PTy> in) {
  ...
  // reuse, diff = in - val
  auto diff = vec64::Sub(absl::MakeConstSpan(in), absl::MakeConstSpan(val));
  ctx->GetConnection()->SendAsync(
      ctx->NextRank(), yacl::ByteContainerView(diff.data(), num * sizeof(PTy)),
      "SetA");
  // extra = diff * key
  auto diff_mac = vec64::ScalarMul(ctx->GetState<Protocol>()->GetKey(),
                                   absl::MakeConstSpan(diff));
  // mac = diff_mac + mac
  vec64::Add(absl::MakeConstSpan(mac), absl::MakeConstSpan(diff_mac),
             absl::MakeSpan(mac));
